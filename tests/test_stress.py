...
async def test_500_rapid_fire_requests():
    ...
    async def make_request(request_id: int):
        response = await agent.arun([
            Message(role=Role.USER, content=f"Request {request_id}")
        ])
        return response.role == Role.ASSISTANT
...
async def test_concurrent_agents_with_memory():
    ...
    async def agent_with_memory(agent_id: int, num_turns: int):
        ...
        for turn in range(num_turns):
            await agent.arun([
                Message(role=Role.USER, content=f"Agent {agent_id} turn {turn}")
            ])
    ...
    print(f"  ✓ 50 agents × 5 turns = {total_requests} requests in {elapsed:.2f}s ({throughput:.1f} req/s)")
    return elapsed, throughput
...
async def test_mixed_workload_realistic():
    ...
    start_time = time.time()
    results = await asyncio.gather(*[mixed_request(req_type, req_id) for req_type, req_id in requests])
    elapsed = time.time() - start_time
    
    assert all(results), "Some requests failed"
    throughput = 100 / elapsed
    
    print(f"  ✓ 100 mixed requests (70% light, 20% medium, 10% heavy) in {elapsed:.2f}s ({throughput:.1f} req/s)")
    return elapsed, throughput
...
async def test_error_handling_under_load():
    ...
    print(f"  ✓ 100 requests with 20% failure rate: {success_count} succeeded, {100-success_count} handled gracefully")
