...
async def test_realistic_customer_support_scenario():
    ...
    print(f"  ✓ Handled {len(conversations)} turn conversation with {len(conversation_log)} tool calls")
...
async def test_streaming_with_async_and_memory():
    ...
    response = await agent.arun(
        [Message(role=Role.USER, content="Generate a report")],
...
async def test_mixed_tool_types_realistic():
    ...
    response = await agent.arun([
        Message(role=Role.USER, content="Query user_123, read config.json, hash it, and call the API")
    ])
...
async def test_large_scale_conversation():
    ...
    for i in range(20):
        await agent.arun([
            Message(role=Role.USER, content=f"Turn {i}: " + "x" * 100)
        ])
